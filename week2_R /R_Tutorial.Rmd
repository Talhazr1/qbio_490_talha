...
```{r setup}
    knitr::opts_knit$set(root.dir = normalizePath("/Users/talhaaccount/Desktop/Qbio_490_R/qbio_490_talha/week2_R /")) 
```
...
```{r}
# write code here
stationTrue <- sum(na_boolean_mask) #Sum of all places where station = TRUE
```
...
```{r}
mask <- c(TRUE, TRUE, FALSE, FALSE, TRUE) #c stands for combine
...
THINK: When using is.na(), are NA values assigned TRUE or FALSE? With this information in mind, what does TRUE mean in your 'na_boolean_mask' vector? Is that data we want to keep, or data we want to ignore. Assigned True, so ignore that data
...
nrow(attenu)
nrow(attenu_cleaned)
#yes since 16 are N/A in station, 166+16 = 182
```
...
```{r}
# write code here
#boxplot(iris$Sepal.Length ~ iris$Species) alternative way
...
Choose any two columns from the iris data set. Create a scatterplot that compares these columns, color coded based on the values relative to the column means. Follow these steps to create your plot:
  ...
  2. Use a series of nested ifelse() statements to create a vector called colors. (HINT: In a nested ifelse(), the "else" condition is to run another ifelse(), and so on. Make sure to close the nest with the correct number of parentheses!) This vector should hold four colors. Assign a color (ex: "blue", "magenta", etc.) to each of the following scenarios:
    a. x is greater than the x column mean and y is greater than the y column mean (blue)
    b. x is greater than the x column mean but y is less than the y column mean (red)
    c. x is less than the x column mean and y is less than the y column mean (green)
    d. x is less than the x column mean but y is greater than the y column mean (yellow)
...
```{r}
# write code here
plMean <- mean(iris$Petal.Length) # mean of x
pwMean <- mean(iris$Petal.Width) # mean of y



colors <- ifelse(iris$Petal.Length > plMean & iris$Petal.Width > pwMean, "blue" , 
                    ifelse(iris$Petal.Length > plMean & iris$Petal.Width < pwMean, "red", 
                           ifelse(iris$Petal.Length < plMean & iris$Petal.Width < pwMean, "green", 
                                  ifelse(iris$Petal.Length < plMean & iris$Petal.Width > pwMean, "yellow", "brown" ))))

as.factor(colors)

print(cor.test(iris$Petal.Length , iris$Petal.Width))

plot(iris$Petal.Length,iris$Petal.Width, col = colors)
abline(h = pwMean, v = plMean)
...
```{r}
# write code here
write.csv(attenu_cleaned, "/Users/talhaaccount/Desktop/attenu_cleaned.csv")
```
...
```{r}
# write code here
attenu_read_in <- read.csv("/Users/talhaaccount/Desktop/attenu_cleaned.csv", header = TRUE)
```
...
```{r}
# write code here
pdf("/Users/talhaaccount/Desktop/attenu_cleaned.pdf")
hist(iris$Petal.Length)
abline(h = 1)
dev.off()
