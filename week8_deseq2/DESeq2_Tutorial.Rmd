...
age_cat_factor <- factor(rna_clinical$age_category)
...
List your chosen variable here:ajcc_pathologic_t
...
rna_clinical$ajcc_pathologic_t <- factor(rna_clinical$ajcc_pathologic_t)

head(stage_at_diag_factor)
head(ajcc_path_t_factor)
```
...
```{r}
na_mask <- ifelse(is.na(rna_clinical$ajcc_pathologic_stage), FALSE, TRUE)
...
dds = DESeqDataSetFromMatrix(countData = rna_counts,
                             ...
                             design = ~age_category + ajcc_pathologic_stage +ajcc_pathologic_t)
...
```{r}
# write code here
geneMask <- ifelse(rna_genes$gene_id %in% results@rownames, T, F)

results <- data.frame(rna_genes$gene_name[geneMask], results@rownames, results$log2FoldChange, results$pvalue, results$padj, -log10(results$padj))
...
```{r}
sig_results <- ifelse(results$pvalue < 0.05, T, F)
```
...
order_indexes <- order(-up_reg_results$y)
...
```{r}
# write code here
rewrite_mask <- ifelse(up_reg_results$y....results.log2foldchange.sig_results. > 1, T, F)
up_reg_results <- up_reg_results[rewrite_mask, ]
```
...
```{r}
# write code here
down_reg <- data.frame(x <- results$gene_name[sig_results], y <- results$log2foldchange[sig_results])
down_order_indexes <- order(down_reg$y)

down_reg <- down_reg[down_order_indexes, ]
neg_rewrite_Mask <- ifelse(down_reg$y....results.log2foldchange.sig_results. < -1, T, F)
down_reg <- down_reg[neg_rewrite_Mask, ]
```
...
```{r}
# write code here
row.names(down_reg) <- c(gsub(" ", "", paste("the_gene_num", 1:124)))
row.names(down_reg) <- c(gsub(" ", "", paste("the_gene_num", 1:274)))
```
...
```{r}
# write code here
write.csv(results, "/Users/talhaaccount/Desktop/Qbio_490_R/qbio_490_talha/analysis_data/brca_deseq2_results.csv", row.names = TRUE)
write.csv(up_reg_results, "/Users/talhaaccount/Desktop/Qbio_490_R/qbio_490_talha/analysis_data/brca_deseq2_up_results.csv", row.names = TRUE)
write.csv(down_reg_results, "/Users/talhaaccount/Desktop/Qbio_490_R/qbio_490_talha/analysis_data/brca_deseq2_down_results.csv", row.names = TRUE)
```
...
top-right genes: significant, upregulated in young
bottom-right genes: not significant, upregulated in young
top-left genes: significant, downregulated in young
bottom-left genes: not significant, downregulated in young
top-middle genes: not significant, same expression
bottom-middle genes: not significant, same expression
...
```{r}
# create plot here
jpeg("/Users/colinyeo/Desktop/School/qbio_490_colin/week8_hw/volcanoplot.jpg")
EnhancedVolcano(results,
    lab = rownames(results),
    x = 'log2foldchange',
    y = 'pvalue',
